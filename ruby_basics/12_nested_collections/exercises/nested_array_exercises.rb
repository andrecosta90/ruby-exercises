...
def count_empty_seats(chart)
  ...
  # chart.flatten().select{ |k| k.nil?}.length

  # chart.flatten().reduce(0) { |a, k| a += k.nil? ? 1 : 0}

  # best solution
  chart.sum { |row| row.count(&:nil?)}

  # word_list.reduce(Hash.new(0)) do |res, item|
...
def find_favorite(array_of_hash_objects)
  ...
  array_of_hash_objects.select{|obj| obj[:is_my_favorite?]}[0]
end
